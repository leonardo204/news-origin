...
- **Fine-tuning** (`finetune_bert_ner.py` v0.4.0): 학습 데이터 임계치(`ner_training_min_samples`) 도달 시 `check_training_readiness`에서 자동 트리거
  ...
  - **finetune 환경변수 (CRITICAL)**: 자동 실행 시 `tasks.py` `trigger_bert_finetune`의 `env_keys` 리스트가 유일한 소스 (Docker SDK는 docker-compose.prod.yml 무시). 새 환경변수 추가 시 `env_keys`에 반드시 추가. `docker-compose.prod.yml` finetune 섹션은 수동 실행 전용.
